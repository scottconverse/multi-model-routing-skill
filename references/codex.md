...
| `--oss --local-provider <ENGINE>` | **Known-fragile local compatibility probe.** Some model chat templates reject Codex's mid-conversation system messages (`System message must be at the beginning`). Use `scripts/local_agent.py` as the primary local tool loop. |
...
# Optional compatibility probe; use scripts/local_agent.py for real local loops
